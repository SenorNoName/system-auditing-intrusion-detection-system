            ...
            for (File file : listOfFiles) {
                if (file.isFile() && file.getName().endsWith(".txt")) {
                    try {
                        ...
                        failedCases.add(file.getName()); // Add the failed file name to the list
                    }
                }
            }
        }

        // Print all failed cases after the loop
        if (!failedCases.isEmpty()) {
            System.out.println("The following files failed to process:");
            for (String fileName : failedCases) {
            System.out.println("File: " + fileName);
            }
            System.out.println("Total failed files: " + failedCases.size());
        } else {
