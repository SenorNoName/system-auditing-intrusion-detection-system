...
//            mode = args[3];
            String logPath = "/home/pxf109/sysrep_exp_env/dataleak";
            String resPath = "/home/pxf109/sysrep_exp_env/dataleak_new_filter";
...
//            String resPath = "C:\\Users\\fang2\\OneDrive\\Desktop\\reptracker\\reptracker\\results\\vpnfilter_filtertest";
            String[] logs = {"dataleak2.txt"};

            ExperimentRunnerCMD_19 er = new ExperimentRunnerCMD_19(logPath, resPath, logs);
            er.run2("clusterall");
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void run2(String mode) throws FileNotFoundException{
        ...
        try{
            ...
            for(Experiment e: experiments_backward){
                ...
                ProcessOneLogCMD_19.run_exp_backward(graphFromLog,oneRes.getAbsolutePath() + "/", "", e.threshold, e.trackOrigin, e.log.getAbsolutePath(), MetaConfig.localIP, e.POI, e.highRP, e.midRP, e.lowRP, e.log.getName().split("\\.")[0], e.detectionSize, e.getInitial(), e.criticalEdges, mode, jsonLog);
...
//                        (name.split("\\.")[0].equals(log.getName().split("\\.")[0]) || name.startsWith(log.getName().split("\\.")[0]+"_")) &&name.endsWith(".property"));
                List<File> propertyFiles = getPropertyFiles(log);
                for(File propertyFile : propertyFiles){
                    String propertyName = propertyFile.getName();
                    if(propertyName.indexOf("backward")!=-1){
            ...
            System.out.println("Finished All the cases!");
        }catch (IOException e){
            e.printStackTrace();
        }
    }
