...
public class InferenceReputation {
    ...
    HashMap<Long, HashMap<Long, Double>> timeWeights; // sink -> source
    ...
    public void calculateWeights(){
        ...
        for(EntityNode n:vertexSet){
            ...
            for (EventEdge inEdge : inEdges){
                ...
                timeWeights.get(n.getID()).put(inEdge.getSource().getID(), timeWeight);
                amountWeights.get(n.getID()).put(inEdge.getSource().getID(), dataWeight);
                structureWeights.get(n.getID()).put(inEdge.getSource().getID(), structureWeight);
            }
        }
        ...
        for(EntityNode n : vertexSet){
            ...
            for (EventEdge e:incoming){
                e.timeWeight = timeWeights.get(n.getID()).get(e.getSource().getID())/timeTotal;
                e.amountWeight = amountWeights.get(n.getID()).get(e.getSource().getID())/amountTotal;
                e.structureWeight = structureWeights.get(n.getID()).get(e.getSource().getID())/structureTotal;

                if(seedSources.contains(e.getSource().getSignature())){ // the seed's structure weight is always 1.0
                    e.structureWeight = 1.0;
                }

                timeWeights.get(n.getID()).put(e.getSource().getID(), e.timeWeight);
                amountWeights.get(n.getID()).put(e.getSource().getID(), e.amountWeight);
                structureWeights.get(n.getID()).put(e.getSource().getID(), e.structureWeight);
            }
        }
        ...
        for(EntityNode n: vertexSet){
            ...
            if(amount < 1e-8){
                ...
                for(EventEdge inEdge: inEdges){
                    ...
                    weights.get(n.getID()).put(inEdge.getSource().getID(), inEdge.weight);
                }
            }else{
                // Calculate total weight in order to normalize
                for(EventEdge inEdge: inEdges){
    ...
    private double getStructureWeightTotal(EntityNode n){
        ...
        for (EventEdge e : graph.incomingEdgesOf(n)){
            total += structureWeights.get(n.getID()).get(e.getSource().getID());
        }
        return total;
    }
    ...
    private double getAmountWeightTotal(EntityNode n){
        double total = 0.0;
        for (EventEdge e : graph.incomingEdgesOf(n)){
            total += amountWeights.get(n.getID()).get(e.getSource().getID());
        }
        return total;
    }
    ...
    private double getTimeWeightTotal(EntityNode n){
        double total = 0.0;
        for(EventEdge e: graph.incomingEdgesOf(n)){
            total += timeWeights.get(n.getID()).get(e.getSource().getID());
        }
        return total;
    }
    ...
    public void calculateWeights_ML(boolean normalizeByInEdges,int mode){
        ...
        for (EntityNode n: vertexSet) {
            ...
            for (EventEdge inEdge: inEdges) {
                timeWeights.get(n.getID()).put(inEdge.getSource().getID(), getTimeWeight(inEdge));
                amountWeights.get(n.getID()).put(inEdge.getSource().getID(), getAmountWeight(inEdge));
                structureWeights.get(n.getID()).put(inEdge.getSource().getID(), getStructureWeight(inEdge));
        ...
        for (EntityNode n: vertexSet) {
            ...
            for (EventEdge inEdge: inEdges) {
                if (seedSources.contains(inEdge.getSource().getSignature())) {
                    ...
                    System.out.println("Normalized structureWeight before hard set: " + structureWeights.get(n.getID()).get(inEdge.getSource().getID()));
                    structureWeights.get(n.getID()).put(inEdge.getSource().getID(), 1.0);
                    System.out.println("Normalized structureWeight after hard set: " + structureWeights.get(n.getID()).get(inEdge.getSource().getID()));
                }
            }
        }
        ...
        for (EntityNode n: vertexSet) {
            inEdges = graph.incomingEdgesOf(n);
            for (EventEdge inEdge: inEdges) {
                inEdge.timeWeight = timeWeights.get(n.getID()).get(inEdge.getSource().getID());
                inEdge.amountWeight = amountWeights.get(n.getID()).get(inEdge.getSource().getID());
                inEdge.structureWeight = structureWeights.get(n.getID()).get(inEdge.getSource().getID());
            }
        }
...
//                System.out.println("After normalization " + inEdge.weight);
    ...
    public void calculateWeights_Fanout(boolean normalizeByInEdges, String resDir){
        ...
        // Initialize weights
        HashMap<Long, HashMap<Long, Double>> fanoutWeights = new HashMap<>(); // sink -> source
        for(EntityNode n1:vertexSet){
            Set<EventEdge> incoming = graph.incomingEdgesOf(n1);
            if (incoming.size() != 0) {
                // n1 -> source not empty
                weights.put(n1.getID(), new HashMap<Long,Double>());
                fanoutWeights.put(n1.getID(), new HashMap<Long,Double>());
                ...
                for (EventEdge inEdge: incoming) {
                    ...
                    fanoutWeights.get(n1.getID()).put(inEdge.getSource().getID(),0.0);
                }
            }
        }
        ...
        for (EntityNode n: vertexSet) {
            inEdges = graph.incomingEdgesOf(n);
            for (EventEdge inEdge: inEdges) {
                fanoutWeights.get(n.getID()).put(inEdge.getSource().getID(), getFanoutWeight(inEdge));
...
//                System.out.println("Before standardize: " + weights.get(sinkNodeID).get(sourceNodeID) + ", After standardize: " + standardizedWeight);
    ...
    private void normalizeWeightsByInEdges(HashMap<Long, HashMap<Long, Double>> weights) {
        for (long sinkNodeID: weights.keySet()) {
            ...
            for (long sourceNodeID: weights.get(sinkNodeID).keySet()) {
                // For each source node, add to weightTotal
                weightTotal += weights.get(sinkNodeID).get(sourceNodeID);
            }

            // Normalize
            if(weightTotal > 1e-8) { // not zero
                double normalizedWeight;
                for (long sourceNodeID: weights.get(sinkNodeID).keySet()) {
                    // For each source node, normalize
                    normalizedWeight = weights.get(sinkNodeID).get(sourceNodeID)/(1.0*weightTotal);
...
//        return Math.exp((-1)*Math.abs(edge.getSize()-detectionSize)/detectionSize);
    ...
    private void printEdgeWeights(EventEdge edge) {
        System.out.println("EventEdge " + edge.getID() + " (" + edge.getSource().getID() + " " + edge.getSource().getSignature() + " ->" + edge.getEvent() + " " + edge.getSink().getID() + " " + edge.getSink().getSignature() + ")" + "\t\t\t timeWeight:" + timeWeights.get(edge.getSink().getID()).get(edge.getSource().getID()) + " amountWeight: " + amountWeights.get(edge.getSink().getID()).get(edge.getSource().getID()) + " structureWeight: " + structureWeights.get(edge.getSink().getID()).get(edge.getSource().getID()) + " finalWeight: " + weights.get(edge.getSink().getID()).get(edge.getSource().getID()));
    }
