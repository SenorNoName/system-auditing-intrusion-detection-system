...
//                weightfile.close();
    ...
    public static void process_forward(String resultDir, String suffix, double threshold, boolean trackOrigin, String logfile, String[] IP, String detection,String[] highRP,String[] midRP,String[] lowRP, String filename,double detectionSize, Set<String> seedSources,String[] criticalEdges, String mode){
        OutputStream weightfile = null;
        try{
            os = new FileOutputStream(resultDir+filename+suffix+"_stats");
            GetGraph getGraph = new GetGraph(logfile, IP);
            ...
            run_exp_forward(orignal, resultDir, suffix, threshold, trackOrigin, logfile, IP, detection,highRP,midRP,lowRP,filename, detectionSize,seedSources,criticalEdges, mode);
...
//                weightfile.close();
    ...
    public static void process_backward(String resultDir, String suffix, double threshold, boolean trackOrigin, String logfile, String[] IP, String detection,String[] highRP,String[] midRP,String[] lowRP, String filename,double detectionSize, Set<String> seedSources,String[] criticalEdges, String mode){
        ...
        try{
            ...
            run_exp_backward(orignal, resultDir, suffix, threshold, trackOrigin, logfile, IP, detection,highRP,midRP,lowRP,filename, detectionSize,seedSources,criticalEdges, mode);
...
//                weightfile.close();
    ...
    // forward analysis + forward propagate
    public static void run_exp_forward(DirectedPseudograph<EntityNode, EventEdge> orignal,String resultDir, String suffix, double threshold, boolean trackOrigin, String logfile, String[] IP, String detection,String[] highRP,String[] midRP,String[] lowRP, String filename,double detectionSize, Set<String> seedSources,String[] criticalEdges, String mode){
        OutputStream weightfile = null;
        try{
            os = new FileOutputStream(resultDir+filename+suffix+"_stats");
            long start = System.currentTimeMillis();
            ForwardAnalysis forwardAnalysis = new ForwardAnalysis(orignal);
            DirectedPseudograph forwardRes = forwardAnalysis.fowardTrack(detection, 10);
            long end = System.currentTimeMillis();
            double timeCost = getTimeCost(start, end);
            System.out.println("Forward time cost is: "+timeCost);
            os.write(("ForwardTrack time cost is: "+timeCost+"\n").getBytes());
            System.out.println("After Forward vertex number is: "+ forwardRes.vertexSet().size() + " edge number: " + forwardRes.edgeSet().size());
            os.write(("After Forward vertex number is: "+ forwardRes.vertexSet().size() + " edge number: " + forwardRes.edgeSet().size()+"\n").getBytes());

            IterateGraph out = new IterateGraph(forwardRes);
            out.exportGraph(resultDir+"Forward_"+filename+suffix);
...
//            os.write(("After CPR1 vertex number is: "+ CPR1.afterMerge.vertexSet().size() + " edge number: " + CPR1.afterMerge.edgeSet().size()+"\n").getBytes());
            ...
            InferenceReputationForward infer = new InferenceReputationForward(split.inputGraph);
...
//            }
            ...
            start = System.currentTimeMillis();
            infer.PageRankIterationForward(highRP,midRP,lowRP,detection);
            end = System.currentTimeMillis();
            timeCost = getTimeCost(start, end);
